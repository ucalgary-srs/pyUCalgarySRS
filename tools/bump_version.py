...
"""
This script will bump the version number to the version specified as a CLI 
parameter by editing the version number in a few files in the repository
automatically.
"""
...
def main():
    # args
    parser = argparse.ArgumentParser(description="Bump the version number for the pyUCalgarySRS library")
    ...
    print("Updating pyucalgarysrs/__init__.py file ...")
    src_filename = "%s/../pyucalgarysrs/__init__.py" % (os.path.dirname(os.path.realpath(__file__)))
    dst_filename = "%s/../pyucalgarysrs/__init__.py.bak" % (os.path.dirname(os.path.realpath(__file__)))
    try:
        # open files for read/write
        shutil.copyfile(src_filename, dst_filename)
        fp_read = open(src_filename, 'r')
