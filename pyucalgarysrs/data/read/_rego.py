...
def __rego_readfile_worker(file, first_record=False, no_metadata=False, start_time=None, end_time=None, quiet=False):
    # init
    images = np.array([])
    metadata_dict_list = []
    is_first = True
    ...
    while True:
        ...
        if (line.startswith(b'#"')):  # type: ignore
            ...
            try:
                ...
                image_np = np.frombuffer(image_bytes, dtype=REGO_DT)  # type: ignore
