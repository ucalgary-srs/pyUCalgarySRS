...
def read(file_list, n_parallel=1, first_record=False, no_metadata=False, start_time=None, end_time=None, quiet=False):
    ...
    images = np.empty([image_height, image_width, list_position], dtype=__BLUELINE_DT)
    ...
    #
    # NOTE: this will need to be moved when we add support for Blueline png.tar data.
...
def __blueline_readfile_worker(file, first_record=False, no_metadata=False, start_time=None, end_time=None, quiet=False):
    # init
    images = np.array([])
    metadata_dict_list = []
    is_first = True
    ...
    while True:
        ...
        if (line.startswith(b'#"')):  # type: ignore
            ...
            try:
                ...
                image_np = np.frombuffer(image_bytes, dtype=__BLUELINE_DT)  # type: ignore
