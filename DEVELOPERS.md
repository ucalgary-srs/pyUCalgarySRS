...
The build downloads about 1.1 GB, so it isn't quick the first time. Files that are already in place are left alone, meaning an interrupted build only fetches what it missed, and intermediate downloads are cached in `tests/test_data/.build_cache`.

Most of the test data is downloaded from the archive as-is. A handful of files can't be, and are regenerated by the script instead:

- the TREx RGB data in the legacy 'unstable' formats (16-bit PGM, and tarballs of PNGs), which the archive has since reprocessed into HDF5
- the 720p 'mode-b3' burst file, for the same reason
- the files that the tests expect to fail reading, which were empty, truncated, or cut short partway through an image frame; most have since been removed from the archive
